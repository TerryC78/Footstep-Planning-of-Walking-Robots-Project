...
Train = Options.Train;  % for training a new set of weights
Test = Options.Test;   % for test classifier
ClassifyTerrain = Options.ClassifyTerrain;   % classify new terrain
...
[WholeTerrain.input, WholeTerrain.num, WholeTerrain.RowNum, WholeTerrain.ColNum, LearnTerrain] = InputWholeTerrain(TerrainFile, Classifier.TerrainWidth, Classifier.TerrainLenth, 0, Options);
...
CostSquare = reshape(WholeTerrain.y, WholeTerrain.RowNum, WholeTerrain.ColNum);  % from vector to square

% ONLY HIGH LEVEL PLANNING NEED CostElement
% based on the learning region, we generate a 10 by 10 matrix
% only when the learning region is the whole map, CostElement is the high
% level cost map
...
CostElement = -(CostElement);  % from reward map to cost map
CostMap = -(CostSquare);  % from reward map to cost map
